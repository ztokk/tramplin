...
#  KONFIGURATION
# ============================================================
HELIUS_API_KEY = "7c8e56df-1ba8-4da2-a79b-9552847490d6"
GITHUB_TOKEN   = "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"
GITHUB_USER    = "ztokk"
...
def load_from_github():
    ...
    try:
        ...
        log("SHA Fehler: " + str(e))
        return None, None
    # Inhalt via Raw URL (kein Base64 Groessenlimit!)
    try:
    ...
    return [
        {
            "rank":      i + 1,
            "wallet":    wallet,
            "total_sol": round(data["total_sol"], 4),
...
def scan():
    ...
    while not stop and page < MAX_PAGES:
        ...
        for tx in txns:
            ...
            if sig in known_sigs or (last_timestamp > 0 and ts < last_timestamp - 60):
                log("Bekannte TX gefunden -> Stop nach Seite " + str(page))
                stop = True
                break
    ...
    log("Neue Wins gefunden: " + str(len(new_wins)))

    if not new_wins:
        log("Keine neuen Wins -> kein Update noetig")
        return
